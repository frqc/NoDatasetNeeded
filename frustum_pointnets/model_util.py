...
NUM_OBJECT_POINT = 512  # max points per object
...
def point_cloud_masking(pts, logits, xyz_only=True):
    """
    Transform the pts into local coordinate and remain pts belonging to the obj only
    :param pts: bs, c, n in frustum
    :param logits: bs, n, 2
    :param xyz_only: bool
    :return:
        object_pts: pts in obj's coordinates an capped by NUM_OBJECT_POINT
        mask_xyz_mean: average (x, y, z) in global coordinates
        mask: belonging to the obj or not
    """
    bs = pts.shape[0]
    n_pts = pts.shape[2]
...
def get_box3d_corners_helper(centers, headings, sizes):
    ...
    corners = torch.cat([x_corners.view(N, 1, 8), y_corners.view(N, 1, 8), z_corners.view(N, 1, 8)], dim=1)  # (N,3,8)
...
def get_box3d_corners(center, heading_residuals, size_residuals):
    ...
    """
    bs = center.shape[0]
    heading_bin_centers = torch.from_numpy(np.arange(0, 2 * np.pi, 2 * np.pi / NUM_HEADING_BIN)).float()  # (12,) (NH,)
...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, logits, mask_label,
        ...
        size_residuals_normalized_loss = huber_loss(size_normalized_dist,
        ...
        corners_3d_pred = torch.sum(
            gt_mask.view(bs, NUM_HEADING_BIN, NUM_SIZE_CLUSTER, 1, 1).float().cuda() * corners_3d, dim=[1, 2])  # (bs,8,3)
        ...
        size_label = mean_sizes + size_residuals_label.view(bs, 1, 3)  # (1,NS,3)+(bs,1,3)=(bs,NS,3)
